...
const builder = new xml2js.Builder({allowSurrogateChars:false});

async function generatePlist(filePath,fileName,ipa_download_url){
    return new Promise((resolve,reject) =>{
        if(fileName.includes('.')){
            fileName = fileName.split('.')[0]+'.plist';//去掉.ipa 后缀名,加上.plist后缀名
        }else{
        ...
        if(checkFile(filePath,fileName)){
            ...
            let templetePath = path.join(__dirname, '..','templete','templete.plist');
            var data = fs.readFileSync(templetePath);
            xmlParser.parseString(data,async function(err,result){
                if(err){
                    resolve(-1);
                }
                result.plist.dict.array.dict.array.dict[0].string[1] = ipa_download_url;
                let xml = builder.buildObject(result);
                console.log(`start create ${fileName} file...`);
                fs.writeFile(path.join(filePath,fileName),xml,'utf8',async function(error){
                    if(error){
